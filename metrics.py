...
class Metrics(object):
    ...
    def make_plots(self, opts, step, real_points,
        ...
        if opts['dataset'] == 'gmm':
            if opts['toy_dataset_dim'] == 1:
                ...
                if opts['plot_kGANs'] == True:
                    self._make_plots_2d_kGAN(opts, step, real_points,
                                    fake_points, weights, prefix)
    ...
    def _make_plots_2d_kGAN(self, opts, step, real_points,
                       fake_points, weights=None, prefix=''):
        max_val = opts['gmm_max_val'] * 2
        if real_points is None:
            real = np.zeros([0, 2])
        ...
        colors = cm.rainbow(np.linspace(0, 1, len(weights)+1))
        plt.clf()
        plt.axis([-max_val, max_val, -max_val, max_val])
        plt.scatter(real[:, 0], real[:, 1], color=colors[0], s=20, label='real')
        idx_plot_colors_end = weights[0]
        idx_plot_colors_start = 0
        for k in range(1, len(weights)+1):
            plt.scatter(fake[idx_plot_colors_start:idx_plot_colors_end, 0], fake[idx_plot_colors_start:idx_plot_colors_end, 1], color=colors[k], s=20, label='fake%d' %k)
        
            if k<len(weights):
                idx_plot_colors_start = idx_plot_colors_end
                idx_plot_colors_end += weights[k]
        ...
        plt.legend(loc='upper left')
        filename = prefix + 'mixture{:09d}.png'.format(step)
        utils.create_dir(opts['work_dir'])
        plt.savefig(utils.o_gfile((opts["work_dir"], filename), 'wb'),
                    format='png')
    ...
    def _return_plots_pics(self, opts, step, real_points,
                         fake_points_list, num_samples_per_gan,weights=None, prefix='', max_rows=16):
        idx_plot_colors_start = 0
        idx_plot_colors_end = 0
        for k in range(opts['number_of_kGANs']):
            idx_plot_colors_start = idx_plot_colors_end
            idx_plot_colors_end += num_samples_per_gan
            fake_points = fake_points_list[idx_plot_colors_start:idx_plot_colors_end] 
            pics = []
            if opts['dataset'] in ('mnist', 'mnist3', 'guitars', 'cifar10'):
                if opts['input_normalize_sym']:
                    if real_points is not None:
            ...
            if k == 0:
                fig = plt.figure(figsize=(opts['number_of_kGANs']*width, height))
    
                filename = prefix + 'comp_mixture{:06d}.png'.format(step)
                utils.create_dir(opts['work_dir'])
            ax = fig.add_subplot(1,opts['number_of_kGANs'],k+1)
            #if self.l2s is None:
            #    fig = plt.figure(figsize=(width, height))#, dpi=1)
            #elif self.Qz is None:
            #    fig = plt.figure(figsize=(width, height + height / 2))#, dpi=1)
