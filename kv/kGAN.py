...
class KGANS(object):
    ...
    def make_step(self, opts, data):
        ...
        pool.close()
        pool.join()
        if False and opts['bagging'] == True:
            ...
            if opts['number_of_kGANs'] == 1:
                ...
                if False and opts['dataset'] == 'mnist' or opts['dataset'] == 'gmm':
    ...
    def _update_data_weights_soft(self, opts, data):
        ...
        if False and opts['number_of_steps_made'] % opts['AIS_every_it'] == 0:
            ais = self._call_ais(opts,True)
        ...
        if opts['number_of_steps_made']%opts["plot_every"] ==0:
            ...
            if False and opts['number_of_steps_made'] % opts['AIS_every_it'] == 0:
                ...
                new_weights[np.arange(len(prob_x_given_gan)), prob_x_given_gan.argmax(1)] = 1. 
                for k in range (0,self._number_of_kGANs):
                    assigned = np.squeeze(self.data_test.data[np.argwhere(new_weights[:,k] == 1)],axis = 1)
    ...
    def _call_ais(self, opts, train):
        ...
        ais_rez = np.zeros(self._number_of_kGANs)
        for k in range (0,self._number_of_kGANs):
            ais_rez[k] = self._call_ais_internal((k,train))
        return ais_rez
        #import pdb
        #pdb.set_trace()
        import pdb
