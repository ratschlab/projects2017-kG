...
class Classifier(object):
    ...
    """
    def __init__(self, opts, data, data_weights = None, model_idx = 0, sess = None):

        # Create a new session with session.graph = default graph
        self._session = sess#tf.Session()
        self._trained = False
        self._data = data
...
class ToyClassifier(Classifier):
    ...
    def _build_model_internal(self, opts):
        ...
        prefix = 'CLASSIFIER' + str(self._model_idx)
        # Placeholders
        ...
        c_logits_real = self.discriminator(
            opts, real_points_ph, prefix=prefix)
        c_logits_fake = self.discriminator(
            opts, fake_points_ph, prefix=prefix, reuse=True)
        c_training = tf.nn.sigmoid(
            self.discriminator(opts, real_points_ph, prefix=prefix, reuse=True))
        ...
        c_vars = [var for var in t_vars if prefix+'/' in var.name]
...
class ImageClassifier(Classifier):
    ...
    def __init__(self, opts, data, data_weights = None, model_idx = 0,  sess = None):
        ...
        Classifier.__init__(self, opts, data, model_idx = model_idx, sess = sess)
    ...
    def _build_model_internal(self, opts):
        ...
        c_logits_real = self.discriminator(
            opts, real_points_ph, is_training_ph, prefix=prefix)
        c_logits_fake = self.discriminator(
            opts, fake_points_ph, is_training_ph, prefix=prefix, reuse=True)
        c_training = tf.nn.sigmoid(
            self.discriminator(opts, real_points_ph, is_training_ph,
                               prefix=prefix, reuse=True))
