...
class Vae(object):
    ...
    """
    def __init__(self, opts, data, weights = None, test_data = None, test_weights = None, model_idx = 0, sess = None):
...
class ToyVae(Vae):
    ...
    def __init__(self, opts, data, weights = None, model_idx = 0, sess = None):

        # One more placeholder for batch norm
        self._is_training_ph = None
        Vae.__init__(self, opts, data, weights, model_idx = model_idx, sess = sess)
    ...
    def generator(self, opts, noise, is_training, reuse=False, return_logits=False):
        ...
        """
        model_idx = str(self._model_idx)
        output_shape = self._data.data_shape # (dim1, dim2, dim3)
        with tf.variable_scope("GENERATOR" + model_idx, reuse=reuse):
 ...
 #       self._saver = saver
    ...
    def compute_ais(self):
        if self._use_second_ais:
            ...
            rez = self._ais_model.evaluate_nchains(self._session, 10)
            return rez[0]  # TODO(damienv)
        else:
            self._ais_model.set_session(self._session)
            batch_size = 64
            train_size = self._data.num_points
