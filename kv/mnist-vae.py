...
#    opts['mnist3_to_channels'] = False # Hide 3 digits of MNIST to channels
    ...
    for step in range(opts["kGANs_number_rounds"]+1):
        ...
        if step%1 == 0:
            fake_points = kG.sample_mixture(opts, 10000)
            filename = opts['work_dir'] + '/samples{:02d}'.format(opts['number_of_steps_made'])
            np.save(filename, fake_points)
